  ...
  <DIV style="text-align: right; clear: both;">  
    <form name='frm' id='frm' method='get' action='./list_by_tourno_search_paging.do'>
      ...
      <c:if test="${param.word.length() > 0 }">
        <button type='button' 
                     onclick="location.href='./list_by_tourno_search_paging.do?tourno=${tourVO.tourno}&word='">검색 취소</button>  
      </c:if>    
    </form>
  </DIV>
      ...
      <c:forEach var="contentsVO" items="${list }" varStatus="status">
        ...
        <tr> 
          <td style='vertical-align: middle; text-align: center;'>
            <c:choose>
              <c:when test="${thumb1.endsWith('jpg') || thumb1.endsWith('png') || thumb1.endsWith('gif')}">
                ...
                <a href="./read.do?contentsno=${contentsno}&now_page=${param.now_page }&word=${param.word }"><IMG src="/contents/storage/${thumb1 }" style="width: 120px; height: 80px;"></a> 
              </c:when>
          ...
          <td style='vertical-align: middle; text-align: center;'>
            ...
            <A href="./delete.do?contentsno=${contentsno}&now_page=${now_page }"><span class="glyphicon glyphicon-trash"></span></A>
          </td>
        </tr>
      </c:forEach>
